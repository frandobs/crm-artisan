...
import { updateClientAction, type UpdateClientState } from '@/lib/actions/clients'
import type { Client } from '@/lib/clients'

export default function EditClientForm({ client }: { client: Client }) {
  const action = updateClientAction.bind(null, client.id)
  const [state, formAction, pending] = useActionState<UpdateClientState, FormData>(action, null)
  ...
  return (
    <div className="flex flex-col min-h-full bg-white">
      ...
      <div className="screen-header gap-3">
        ...
        <h1 className="screen-title">Edit client</h1>
      </div>

      <form action={formAction} onSubmit={handleSubmit} className="flex flex-col gap-5 p-4 pb-10">
